...
/**
 * Builds the standalone `bungee-bundle` plugin: a shaded, drop-in BungeeCord
 * plugin that jar-in-jars every MagicUtils module so proxy owners install one
 * runtime. This mirrors [MagicUtilsVelocityBundlePlugin] (BungeeCord is a plain
 * JVM plugin like Velocity/Bukkit, so it shades via the `bundleShadow`
 * configuration rather than jar-in-jarring like the loader-based Fabric/NeoForge
 * bundles).
 *
 * Unlike bukkit/fabric there is no per-target BungeeCord API: the API is
 * decoupled from the Minecraft version, so the version comes from the
 * `bungeeApiVersion` gradle property (default
 * [MagicUtilsConsumerBungeePlugin.DEFAULT_BUNGEE_API]).
 */
class MagicUtilsBungeeBundlePlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        with(project) {
            ...
            // BungeeCord API on the compile classpath for the plugin entrypoint
            // (Plugin/ProxyServer). It is provided by the proxy at runtime, so it
            // stays out of the bundle jar (compileOnly, not bundleShadow). The
            // plugin descriptor is the hand-authored plugin.yml below.
            dependencies.add("compileOnly", "net.md-5:bungeecord-api:$bungeeApiVersion")

            // Fill plugin.yml's ${version} from the build version, the same token
            // flow bukkit-bundle/velocity-bundle use.
            tasks.named("processResources", ProcessResources::class.java).configure { resources ->
                resources.inputs.property("version", version)
                resources.filesMatching(listOf("plugin.yml")) { details ->
