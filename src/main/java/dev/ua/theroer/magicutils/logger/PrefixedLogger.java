...
public class PrefixedLogger {
    ...
    public void send(LogLevel level, Object message, Player player) {
        if (!enabled)
            return;
        if (message instanceof String) {
            Logger.send(level, formatMessage((String) message), player, null, Target.CHAT, false);
        } else {
            Logger.send(level, message, player, null, Target.CHAT, false);
        }
    }
    ...
    public void sendToConsole(LogLevel level, Object message) {
        if (!enabled)
            return;
        if (message instanceof String) {
            Logger.send(level, formatMessage((String) message), null, null, Target.CONSOLE, false);
        } else {
            Logger.send(level, message, null, null, Target.CONSOLE, false);
        }
    }
    ...
    public void sendToPlayers(LogLevel level, Object message, Collection<? extends Player> players) {
        if (!enabled)
            return;
        if (message instanceof String) {
            Logger.send(level, formatMessage((String) message), null, players, Target.CHAT, false);
        } else {
            Logger.send(level, message, null, players, Target.CHAT, false);
        }
    }
