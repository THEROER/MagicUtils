    ...
    public static List<CommandArgument> getArguments(Method method) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            for (Annotation annotation : paramAnnotations[i]) {
                ...
                if (annotation instanceof OptionalArgument) {
                    optional = true;
                }
            ...
            if (!suggestions.isEmpty()) builder.suggestions(suggestions);
            if (permission != null) builder.permission(permission);
            if (permissionCondition != null) builder.permissionCondition(permissionCondition);
            if (permissionMessage != null) builder.permissionMessage(permissionMessage);
